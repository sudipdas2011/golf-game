...
@export var camera_offset : float = 4.0 # Acts as your default Spring Arm Length
...
# Look Rotations (Target vs Smoothed Values)
...
func _ready() -> void:
	...
	# Apply your camera_offset export variable straight to the spring arm length
...
func _process(delta: float) -> void:
	...
	# 1. Smoothly follow the player's position anchor
	...
	# 2. Smoothly lerp looking angle values
	...
	# 3. Rotate the SpringArm3D node (Physics engine moves the child Camera3D instantly)
...
func camera_ranging(radius: float) -> void:
	# Debug visualization telemetry readouts
...
func camera_fov(delta: float) -> void:
	...
	CAMERA.fov = lerp(CAMERA.fov, 75.0 + (current_speed * fov_fac), 0.1)
